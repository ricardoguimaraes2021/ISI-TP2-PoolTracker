...
public class WaterQualityController : ControllerBase
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)] // Para quando retorna null
    public async Task<ActionResult> GetLatest([FromQuery] string? poolType = null)
    {
        if (!string.IsNullOrEmpty(poolType))
        {
            // Parse string to enum (case-insensitive)
            if (Enum.TryParse<PoolType>(poolType, true, out var poolTypeEnum))
            {
                var measurement = await _waterQualityService.GetLatestMeasurementAsync(poolTypeEnum);
                // Sempre retornar HTTP 200, mesmo quando não há medições
                // Retornar null explicitamente como JSON para evitar HTTP 204
                if (measurement == null)
                {
                    // Forçar retorno de JSON null em vez de HTTP 204
            ...
            {
                return BadRequest($"Invalid poolType. Valid values are: {string.Join(", ", Enum.GetNames(typeof(PoolType)))}");
            }
        }
