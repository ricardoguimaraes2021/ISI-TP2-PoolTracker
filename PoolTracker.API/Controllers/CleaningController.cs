...
public class CleaningController : ControllerBase
{
    ...
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    public async Task<ActionResult<CleaningDto>> RecordCleaning([FromBody] RecordCleaningRequestDto? request)
    {
        try
        {
            if (request == null || string.IsNullOrEmpty(request.CleaningType))
            {
                return BadRequest(new { error = "CleaningType é obrigatório" });
            }

            // Parse enum (case-insensitive)
            if (!Enum.TryParse<CleaningType>(request.CleaningType, true, out var cleaningType))
            {
                return BadRequest(new { error = $"Invalid CleaningType. Valid values are: {string.Join(", ", Enum.GetNames(typeof(CleaningType)))}" });
            }

            var recordRequest = new RecordCleaningRequest
            {
                CleaningType = cleaningType,
                Notes = request.Notes
            };

            Console.WriteLine($"RecordCleaning called with CleaningType: {recordRequest.CleaningType}, Notes: {recordRequest.Notes}");
            var cleaning = await _cleaningService.RecordCleaningAsync(recordRequest);
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(500, new { error = "Erro ao registar limpeza", message = ex.Message });
        }
    }
