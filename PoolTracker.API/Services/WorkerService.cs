...
public class WorkerService : IWorkerService
{
    ...
    {
        var lastWorker = _context.Workers
            .Where(w => w.WorkerId.StartsWith("W") && w.WorkerId.Length == 5)
            .OrderByDescending(w => w.WorkerId)
            .FirstOrDefault();

        int nextId = 1;
        if (lastWorker != null && int.TryParse(lastWorker.WorkerId.Substring(1), out int lastId))
        {
            nextId = lastId + 1;
        }
    ...
    {
        var workerId = request.WorkerId ?? GenerateWorkerId();
